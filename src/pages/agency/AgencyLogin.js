...
export default function AgencyLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      ...
      if (role === 'agency_admin' || role === 'agency_staff') {
        ...
        setError('Ce portail est réservé aux utilisateurs agence. Accès non autorisé.');
        // Log them back out
  ...
  return (
    <div style={{
      minHeight: '100vh', background: 'linear-gradient(135deg, #003d78 0%, #005096 50%, #0077cc 100%)',
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          ...
          <p style={{ fontSize: 14, color: '#6b7280', margin: 0 }}>
            Easy2Book B2B — Connectez-vous à votre espace agence
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            {loading ? <><Loader size={16} style={{ animation: 'spin 1s linear infinite' }} /> Connexion...</> : 'Se connecter'}
          </button>
        </form>
